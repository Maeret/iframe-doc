...
<details>
  ...
  1.  (optionally) Generate unique `token1` and `sign` (see
      [Request signature](/iframe-doc/backend/sign)).
  1.  Call [/GetBalance](#operation/getBalance) method.
  1.  IFRAME Service will send a [/GetUserData](#operation/getUserData)
...
<details>
  ...
  1.  (optionally) Generate unique `token1` and `sign` (see
      [Request signature](/iframe-doc/backend/sign)).
  1.  Call [/TransferFunds](#operation/transferFunds) method with operation
