...
```
https://{domain}/{lng}/{path}?lng={lng}&token={token}&sign={sign}&cyber_type={cyber_type}
```
...
| live           | Live
| line           | Sports 
| office/history | payment History. See also [payment History API](/betHistory)
| results        | Results
...
> `/office/history` is only to authorized users: only with a valid `token`.
...
Esports display control key.

:::info
[Esports controls](/frontend/cyber).
:::
